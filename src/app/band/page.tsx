...
export default function BandPage() {
  return <><ShrinkingHeader /><main className="min-h-screen bg-[#050505] px-4 pb-20 pt-36 text-white sm:px-6 lg:px-8"><section className="mx-auto max-w-7xl">
    ...
    <div className="mt-12 grid gap-6 md:grid-cols-2">{bandMembers.map((member, index) => <article key={member.name} className="group grid overflow-hidden border border-white/12 bg-[#101010] shadow-[0_20px_50px_rgba(0,0,0,0.24)] sm:grid-cols-[minmax(11rem,0.72fr)_1.28fr]">
      <div className="relative min-h-80 overflow-hidden bg-[#171717] sm:min-h-full">{'image' in member ? <Image src={member.image} alt={member.imageAlt} fill sizes="(min-width: 768px) 25vw, 100vw" className={`object-cover ${member.imagePosition} transition duration-500 group-hover:scale-[1.025]`} /> : <div className="absolute inset-0 flex flex-col items-center justify-center bg-[radial-gradient(circle_at_50%_30%,rgba(255,39,0,0.22),transparent_52%),linear-gradient(145deg,#1b1b1b,#090909)] p-8 text-center"><span aria-hidden="true" className="text-8xl font-black text-white/8">G</span><span className="mt-4 text-xs font-black uppercase tracking-[0.22em] text-white/48">Photo coming soon</span></div>}</div>

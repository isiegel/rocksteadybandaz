...
function createEventGraph(upcoming: ReturnType<typeof upcomingShows>) {
  return {
...
export default async function ShowsPage() {
  ...
  const eventGraph = createEventGraph(upcoming);

  return <><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(eventGraph).replace(/</g, '\\u003c') }} /><main className="min-h-screen bg-[#050505] px-4 pb-20 pt-36 text-white sm:px-6 lg:px-8"><section className="mx-auto max-w-7xl">
    <p className="text-sm font-black uppercase text-[#37d67a]">Around town</p><h1 className="mt-3 max-w-4xl text-5xl font-black leading-tight sm:text-6xl">Phoenix live music built for rooms that love a live band.</h1><p className="mt-5 max-w-3xl text-lg leading-8 text-white/72">Find the next Rock Steady show, then bring somebody who knows every word.</p>
